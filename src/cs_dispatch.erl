...
init(_Args) ->
    cs_io:debug("cs_dispatch init", 0, 16),
    {ok, #state{}}.
...
handle_cast(enter_insert_mode, State = #state{mode = insert}) ->
    cs_io:clear(10, 17),
    cs_io:debug("Entering insert while in insert", 0, 13),
    {noreply, State};
handle_cast(Req, State) ->
    cs_io:clear(10, 17),
    cs_io:debug(Req, 0, 10),
    {noreply, State}.
